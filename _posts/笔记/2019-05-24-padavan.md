---
title: Padavan 上SS的频繁报错的问题及报错解决办法
tags: [路由器, padavan, 老毛子]
---

路由器是 K2 刷的老毛子的 Padavan，科学上一下网

有一个关于配置问题，SS 频繁报错重启的情况，过上两三分钟，首页网络地图下的 ss 图标就出现错误标志

看到配置有检查 SS 状态,可以关闭定期检测，这样就不会提示这种问题了，出现这种问题不会影响正常使用
...
都是从 github 拉取的, 网络不好的情况下, 根本拉不到, 这样重试就要花 3 分钟重启 ss

后面我下载对应文件上传后, 修改了它的脚本, 重启 ss 只要几秒钟

```
// 修改下载相关脚本
scp admin@192.168.123.1:/etc/storage/script/Sh15_ss.sh ~/Downloads/

// 上传 *.txt 到指定目录
scp ~/Downloads/chnroute.txt admin@192.168.123.1:/etc/storage/tmp_chnroute.txt
scp ~/Downloads/gfwlist.txt admin@192.168.123.1:/etc/storage/tmp_gfwlist.txt

// 修改文件,上传
scp ~/Downloads/Sh15_ss.sh admin@192.168.123.1:/etc/storage/script/Sh15_ss.sh
```
...
if [ "$ss_updatess" = "0" ] || [ "$ss_updatess2" = "1" ] ; then
	...
	if [ "$ss_3p_enable" = "1" ] ; then
		if [ "$ss_3p_gfwlist" = "1" ] ; then
			...
			cat /etc/storage/tmp_gfwlist.txt | sort -u |
					sed '/^$\|@@/d'|
...
if [ "$ss_updatess" = "0" ] || [ "$ss_updatess2" = "1" ] ; then
	...
	ip_list="ss_spec_dst_sh"
		...
		cat /etc/storage/tmp_chnroute.txt > /tmp/ss/chnroute.txt
else
